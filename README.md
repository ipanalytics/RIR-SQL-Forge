...
public RIR FTP/HTTPS
 ...
 SQLite bulk writer     ---> normalized tables
        |
        v
 flatten SQL + CSV      ---> release artifacts
```
...
- SQLite output using `modernc.org/sqlite` with no CGO requirement.
- Flat CSV export for downstream systems that do not need relational tables.
...
This is the normal production path. The compiler downloads public RIPE, APNIC, and AFRINIC source files, parses them, and writes SQLite/CSV outputs.
...
The SQLite database contains normalized source tables:
...
- The CSV is a flattened operational view; use SQLite when preserving joins matters.
...
6. compresses SQLite and CSV outputs
7. creates or updates a GitHub Release tagged `db-YYYY-MM-DD`
